...
const int PUZNUM = 6; //Puzzle number
  ...
  if (pressed) {
    if (millis() - interrupt_time > 2000) {
      if (!seq_cmp2(yourSequence, sequence)) {
        for (int t = 0; t < 3; t++)buzzer();
        seq_clear(yourSequence);
        Serial.println("Puzzle not solved, sending 6-false");
        puzzleSolved(false);   //when pin5 is pulled low, set coil to true in ClueControl
        Pin5Hold = true;
        solved = 0;
        interrupt_time = millis();
        ...
        if (solved)
        {
          Serial.println("Puzzle solved, sending 6-true");
          puzzleSolved(true);   //when pin5 is pulled low, set coil to true in ClueControl
          Pin5Hold = true;
          solved = 0;
        }
      }
